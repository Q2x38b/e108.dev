/* eslint-disable react-refresh/only-export-components, react-hooks/refs -- vendored from interior.dev; exports its hook beside the component and mirrors callbacks into refs during render by design */
// Blur-up image, from interior.dev (blur-up-image). Adapted for this site:
// framer-motion imports, React's own layout effect (no SSR here), the
// placeholder tint comes from the site's tokens, and width/height are
// optional so a masonry tile can take the image's natural size.
...
export function useBlurUpImage({ src, srcSet, onReady, onError }: UseBlurUpImageOptions) {
  ...
  const [state, setState] = useState<{ status: BlurUpStatus; instant: boolean }>({
    status: 'loading',
    instant: false,
  })
  ...
  useLayoutEffect(() => {
    ...
    const set = (status: BlurUpStatus, instant: boolean) =>
      setState((prev) => (prev.status === status && prev.instant === instant ? prev : { status, instant }))
    ...
    // Already in the browser cache: show it at once, no develop animation
...
export type BlurUpImageProps = {
  src?: string
  alt: string
  /** Aspect ratio of the box. Omit both to let the image set its own height. */
  width?: number
  height?: number
  placeholder?: string
  color?: string
...
export function BlurUpImage({
  ...
  return (
    <div
      ...
      style={{
        aspectRatio: sized ? `${width} / ${height}` : undefined,
        borderRadius: radius,
