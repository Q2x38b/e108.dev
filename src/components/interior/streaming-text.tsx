/* eslint-disable react-refresh/only-export-components, react-hooks/set-state-in-effect -- vendored from interior.dev; exports its hook beside the component by design */
// Token-by-token text reveal with a caret, from interior.dev (streaming-text).
// Adapted for this site: imports from framer-motion (already a dependency)
// and no Tailwind colour classes, so the text and caret inherit whatever
// colour the surrounding CSS gives them.
...
export function useStreamingText({
  ...
  return { tokens, index, total, status, visible, start, pause, skip, reset }
}
...
export function StreamingText({
  ...
  const caret = (
    <span aria-hidden className="relative inline-block h-[1.1em] w-0 align-[-0.22em]">
      <motion.span
        className="absolute inset-y-0 left-px block w-[2px]"
        style={{ background: 'currentColor' }}
        initial={false}
        animate={blink ? { opacity: [1, 1, 0, 0] } : { opacity: done ? 0 : 1 }}
        transition={
          blink
            ? { duration: 1.06, times: [0, 0.45, 0.5, 0.95], repeat: Infinity, ease: 'linear' }
            : CROSSFADE
        }
      />
    </span>
  ...
  return (
    <div role="group" aria-label={label} aria-busy={status === 'streaming'} className={className}>
      {/* The full text sits invisibly to reserve the final size, so nothing
          reflows as the visible copy streams in on top of it. */}
      ...
      {showSkip ? (
        <div className="mt-2.5 flex justify-end">
          <button
            ...
            className="inline-grid h-7 place-items-center rounded-[6px] border px-2.5 text-[11.5px] font-medium"
